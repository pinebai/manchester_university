...
!
      root1=0;root2=0;delr=1.e-7
      if(rub.lt.0.2e-6)then
        rrub=50.e-6
      endif
      if(rlb.lt.0.2e-6)then
        rrlb=0.2e-6
      else
        root1=rlb
        rrlb=rlb
...
!
      do r=rrlb,rrub,delr
        terml=flagu((r-delr)/rnorm,3.)
        termc=flagu(r/rnorm,3.)
        termu=flagu((r+delr)/rnorm,3.)
...
200   continue
!
      if(root1.gt.tiny.and.root2.gt.tiny)then
        rlbn=root1/rnorm
        rubn=root2/rnorm
!
        icount=0;icountmx=15
300     continue
        icount=icount+1
!
        call qalt(flagu,3.,rlbn,rubn,qq3n)
        qnorm=q0(i,j,k)/(1./rnorm)**(3.)
        qq3=qq3n*qnorm
        ratio=qq3/q3(i,j,k)
!
        if(ratio.lt.0.95 .and.icount.lt.icountmx)then
          if(ratio.gt.0.8)rubn=rubn+0.05
          if(ratio.lt.0.8)rubn=rubn+0.15
          goto 300
        elseif(ratio.gt.1.05 .and.icount.lt.icountmx)then
          if(ratio.lt.1.2)rubn=rubn-0.05
          if(ratio.gt.1.2)rubn=rubn-0.15
          goto 300
        endif
!
